...
function Maintenance() {
  ...
  const [form, setForm] = useState({
    vehicle: '', type: '', description: '', cost: '', date: '', nextServiceDate: '', status: 'Scheduled', workshop: ''
  });
  ...
  const fetchAll = async () => {
    try {
      ...
      const [maintData, vehiclesData] = await Promise.all([
        maintRes.json(), vehiclesRes.json()
      ]);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          {loading ? (
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan="7" className="px-4 py-8 text-center text-gray-400">No maintenance records found</td></tr>
                ) : (
                  filtered.map((r) => (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl overflow-y-auto max-h-screen">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-800">Add Maintenance Record</h3>
